...
from apps.orders.models import APIMethodEnum, ORDER_SUB_STATUS, RECHARGE_PROCESSING, RECHARGE_FAILED, \
    RECHARGE_COMPLETED, PostpaidAccountBalance, DU_POSTPAID, MBME
from apps.orders.serializers import PostpaidAccountBalanceSerializer
...
class DUPostpaidAPIClient:
    ...
    def save_balance_in_db(self, resp, recharge_number):
        ...
        if resp.get("responseCode") == "000":
            balance = resp.get("responseData").get("amount")
            customer_name = resp.get("responseData").get("custName")
            obj, created = PostpaidAccountBalance.objects.update_or_create(
    ...
    def get_balance_from_db(self, recharge_number):

        qs = PostpaidAccountBalance.objects.filter(
            service_type=DU_POSTPAID,
            service_provider=MBME,
            recharge_number=recharge_number,
